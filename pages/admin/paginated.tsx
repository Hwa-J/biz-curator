...
const Paginated: FC = () => {
  ...
  const fetchTodos = (pageNumber = 0) =>
    fetch(`http://43.201.195.195:8080/api/admins/applications/cancellations`).then((res) => res.json());
  ...
  return (
    <>
      {data?.todos && data.todos.map((todo: any) => <p key={todo.applicationId}>{todo.cost}</p>)}
